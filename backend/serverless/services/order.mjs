...
import { getProductsByIds, enrichItems, calculateTotal, trimFields } from "../utils/orderHelpers.mjs";
import { generateId } from "../utils/index.mjs";

export const createOrder = async ({ items, userComment = "", staffComment = "" }) => {
  ...
  // Enrich items with product data
	...
	const trimmedItems = fullItems.map(item => ({
		...item,
		extras: trimFields(item.extras, ["img", "stock", "summary", "description", "includeDrink"])
	}));
